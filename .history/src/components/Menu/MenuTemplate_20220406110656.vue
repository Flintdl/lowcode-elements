      ...
      <div class="sf-menu-body">
        <div class="sf-row sf-px-4" style="visibility: visible">
          <div
            ...
            class="sf-col-4 sf-text-center sf-form-group"
          >
            <div
              @click="showContent"
              :dataRef="tab.ref"
              class="sf-bg-primary sf-p-2 sf-text-white sf-font-weight-bold sf-border-radius"
            >
              {{ tab.label }}
            </div>
          </div>
        </div>
        <ul class="sf-menu-ul" ref="template-collapse" style="display: none">
...
export default {
  ...
  methods: {
    showContent(e) {
      const rs = e.currentTarget.getAttribute("dataRef");
      for (let i = 0; i < this.tabs.length; i++) {
        const element = this.tabs[i];
        console.log(this.$refs[element.ref]);
        if (this.$refs[rs]) {
          this.$refs[rs].classList.toggle("open");
          if (!this.$refs[rs].classList.contains("open")) {
            this.$refs[rs].classList.remove("open");
          } else {
            this.$refs[rs].classList.add("open");
          }
        }
      }
    },
